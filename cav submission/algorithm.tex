...
\Input{$G=(V_t\cup V_f,E)$, the synthesis graph; $\tau \in V_t$, the type to synthesize; $C_{max}$, the maximum allowable cost; $L$, the number of snippets to synthesize}
...
\ForEach{$\sigma\in V_t'$}{
  \Snips[$\sigma$] $\longleftarrow$ \ref{proc:getExpressions}($G'$, \Snips, $\sigma$, $C_{max} - $ \Dist{$\sigma$}, $L$) \;
}
